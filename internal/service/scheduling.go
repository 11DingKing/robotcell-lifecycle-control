...
func (s *Scheduling) Request(ctx context.Context, window schedule.WorkWindow) (schedule.WorkWindow, error) {
	if _, err := RequireRole(ctx, identity.RoleLineManager, identity.RoleIntegrator); err != nil {
...
func (s *Scheduling) Approve(ctx context.Context, id, expected int64, qualification, requestID string) (schedule.WorkWindow, error) {
	principal, err := RequireRole(ctx, identity.RoleLineManager, identity.RoleSafetyOfficer)
	if err != nil {
		return schedule.WorkWindow{}, err
	}
...
func (s *Scheduling) Cancel(ctx context.Context, id, expected int64, reason, requestID string) (schedule.WorkWindow, error) {
	principal, err := RequireRole(ctx, identity.RoleLineManager, identity.RoleOperator, identity.RoleIntegrator)
	if err != nil {
		return schedule.WorkWindow{}, err
	}
	if reason == "" {
