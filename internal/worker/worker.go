...
func (w *Worker) drainOne(ctx context.Context) {
	...
	now := w.clock.Now()
	if err == nil {
		if w.store.CompletionIsExternallyVisible(job.Kind) {
			if yieldErr := w.store.YieldRecoveryLease(ctx, job.ID, w.owner, now); yieldErr != nil {
				w.logger.Error("recovery lease yield failed", "job_id", job.ID, "error", yieldErr)
				return
			}
		}
