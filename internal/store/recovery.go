...
func (s *Store) CreateRecoveryJob(ctx context.Context, job recovery.Job) (recovery.Job, error) {
	...
	if err != nil {
		if isConflict(err) {
			return s.reuseRecoveryJob(ctx, job)
		}
...
func (s *Store) reuseRecoveryJob(ctx context.Context, requested recovery.Job) (recovery.Job, error) {
	existing, err := s.FindRecoveryByKey(ctx, requested.IdempotencyKey)
	if err != nil {
		return recovery.Job{}, fmt.Errorf("read idempotent recovery job: %w", err)
	}
	return existing, nil
}
