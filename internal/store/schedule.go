...
func (s *Store) StageCellForWindow(ctx context.Context, principal identity.Principal, windowID int64, requestID string, now time.Time) error {
	window, err := getWindow(ctx, s.db, windowID)
	if err != nil {
		return err
	}
	cell, err := getCell(ctx, s.db, window.CellID)
	if err != nil {
		return err
	}
	_, err = s.TransitionCell(ctx, principal, cell.ID, cell.Version, lifecycle.CellScheduled, "work window approved", requestID, now)
	return err
}
