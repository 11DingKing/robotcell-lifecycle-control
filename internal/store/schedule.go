...
func (s *Store) ReleaseWindowReservations(ctx context.Context, id int64, now time.Time) error {
	result, err := s.db.ExecContext(ctx, `UPDATE resource_reservations SET released_at=? WHERE window_id=? AND released_at IS NULL`, encodeTime(now), id)
	if err != nil {
		return fmt.Errorf("release reservations before cancellation: %w", err)
	}
	if _, err = result.RowsAffected(); err != nil {
		return fmt.Errorf("count released reservations: %w", err)
	}
	return nil
}
