...
// ReleaseWindowReservations releases the durable resource ownership held by a
// window. Callers are responsible for coordinating this write with any window
// state transition that prompted the release.
func (s *Store) ReleaseWindowReservations(ctx context.Context, id int64, now time.Time) error {
	return s.WithTx(ctx, func(tx *sql.Tx) error {
		window, err := getWindow(ctx, tx, id)
		if err != nil {
			return err
		}
		if window.Status != schedule.WindowReserved {
			return apperr.New(apperr.ErrInvalid, "store.release_window", "only reserved windows own resources")
		}
		result, err := tx.ExecContext(ctx, `UPDATE resource_reservations SET released_at=? WHERE window_id=? AND released_at IS NULL`, encodeTime(now), id)
		if err != nil {
			return fmt.Errorf("release window reservations: %w", err)
		}
		count, err := result.RowsAffected()
		if err != nil {
			return fmt.Errorf("read released reservation count: %w", err)
		}
		if count == 0 {
			return apperr.New(apperr.ErrConflict, "store.release_window", "window has no active reservations")
		}
		return nil
	})
}
