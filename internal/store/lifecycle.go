...
func (s *Store) ReleaseCellReservations(ctx context.Context, cellID int64, now time.Time) error {
	return s.WithTx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, `UPDATE resource_reservations SET released_at=? WHERE window_id IN (SELECT id FROM work_windows WHERE cell_id=?) AND released_at IS NULL`, encodeTime(now), cellID)
		return err
	})
}
