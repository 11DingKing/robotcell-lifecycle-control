...
func (s *Store) RecordLogoutSuccess(ctx context.Context, principal identity.Principal, now time.Time) error {
	event, err := audit.New(principal.UserID, string(principal.Role), "session.logout", "session", principal.SessionID, "logout:"+principal.SessionID, audit.ResultSucceeded, map[string]any{"revoked": true}, now)
	if err != nil {
		return fmt.Errorf("build logout audit: %w", err)
	}
	if _, err = s.AppendAudit(ctx, event); err != nil {
		return fmt.Errorf("append logout audit: %w", err)
	}
	return nil
}
